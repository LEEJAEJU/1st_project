...
from sensor_data_fuc import post_set_data_classify_fuc, data_check
...
def login_page() -> render_template:
    ...
    Returns:
        render_template(str): String_containing_HTML_code
    """
...
def new_worker_registration() -> render_template:
    """_summary_

    Returns:
        render_template(str): _description_
    """
    if request.method == "POST":
...
@app.route("/insert/test", methods=["Get"])
def test():
    view_table = ["0", "0", "0", "0", "0", "0", "0", "0"]
    view_table[0] = "1"  # 해당하는 표 작동용
    data_list = [[], [], [], [], [], [], []]
    data_list = data_check("0")
    print(data_list)
    response = make_response(data_list, 200)
    print(type(response))
    return response
